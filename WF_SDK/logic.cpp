...
std::vector<unsigned short> wf::Logic::record(Device::Data *device_data, int channel) {
    ...
    */
    ...
    while (true) {
        ...
        if (status == stsDone) { // only activates once the data acquisition is triggered AND FINISHED
    ...
    if (FDwfDigitalInStatusData(device_data->handle, buffer.data(), 2 * data.buffer_size) == 0) { // 2 * because 2 bytes are in a short
        device.check_error(device_data);
    }
